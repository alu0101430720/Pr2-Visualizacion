...
@asset_check(
    asset=plot_covid_contratos_2020_vs_2019,
    description=(
        "[G-COVID1] Verifica que contratos2019.csv y contratos2020.csv existen, "
        "tienen los 12 meses completos, ambos sexos y volumen mínimo por mes."
    ),
)
def check_datos_covid_contratos(context):
    '''
    Gestalt — Continuidad [G-COVID1]: sin los 12 meses completos de ambos años
    las líneas presentan saltos que el lector confunde con caídas reales.
    Similitud: si falta algún sexo en algún mes, el panel de barras genera
    valores atípicos silenciosos que distorsionan la narrativa de género.
    '''
    import pandas as pd
 
    data_dir  = os.path.join(config.TARGET_DIR, config.DATA_P5_DIR)
    passed    = True
    report_md = "### [G-COVID1] Precondiciones: covid_contratos_2020_vs_2019\n\n"
    ...
    for año, fname in [(2019, "contratos2019.csv"), (2020, "contratos2020.csv")]:
        ...
        df  = pd.read_csv(fpath, sep=sep, dtype={"contratos": float})
        ...
        ok    = set(meses) == set(range(1, 13))
        passed = passed and ok
        report_md += f"- `{fname}` 12 meses: {'🟢' if ok else '🔴'} ({meses})\n"
 
        # Ambos sexos en todos los meses
        pivote = df.groupby(["mes","sexo"])["c"].sum().unstack("sexo").fillna(0)
        sin_h  = [m for m in range(1,13) if pivote.get("Hombres", pd.Series()).get(m, 0) == 0]
        sin_m  = [m for m in range(1,13) if pivote.get("Mujeres",  pd.Series()).get(m, 0) == 0]
        ok2 = len(sin_h) == 0 and len(sin_m) == 0
        passed = passed and ok2
        report_md += (
            f"- `{fname}` ambos sexos completos: {'🟢' if ok2 else '🔴'}"
            f" (sin H: {sin_h or '–'}, sin M: {sin_m or '–'})\n"
        )
 
        # Volumen mínimo por mes ≥ 100
        vol_min = int(df.groupby("mes")["c"].sum().min())
        ok3 = vol_min >= 100
        passed = passed and ok3
        report_md += (
            f"- `{fname}` vol. mínimo mensual ≥ 100: {'🟢' if ok3 else '🔴'}"
            f" ({vol_min:,})\n"
        )
    ...
    return AssetCheckResult(
        passed=bool(passed),
        severity=AssetCheckSeverity.WARN,
        metadata={"G_COVID1_Datos": MetadataValue.md(report_md)},
    )
...
@asset_check(
    asset=plot_covid_contratos_2020_vs_2019,
    description=(
        "[G-COVID2] Verifica que las columnas y separadores de contratos2019.csv "
        "y contratos2020.csv son compatibles para el join mensual."
    ),
)
def check_cobertura_contratos_2019(context):
    '''
    Gestalt — Similitud [G-COVID2]: si los separadores o los nombres de columna
    difieren entre 2019 y 2020, el merge produce NaN en todos los meses de 2019,
    haciendo que la variación relativa aparezca como −100% en todos los meses.
    Es un error catastrófico y completamente silencioso sin este check.
    '''
    import pandas as pd
 
    data_dir  = os.path.join(config.TARGET_DIR, config.DATA_P5_DIR)
    passed    = True
    report_md = "### [G-COVID2] Compatibilidad estructura 2019 vs 2020\n\n"
    COLS_REQ  = {"fecha", "sexo", "contratos"}
    ...
    for año, fname in [(2019, "contratos2019.csv"), (2020, "contratos2020.csv")]:
        ...
        sep  = _detect_sep(fpath)
        seps[fname] = sep
        cols = set(pd.read_csv(fpath, sep=sep, nrows=0)
                   .columns.str.strip().str.lower())
 
        faltantes = COLS_REQ - cols
        ok = len(faltantes) == 0
        passed = passed and ok
        report_md += (
            f"- `{fname}` columnas requeridas: {'🟢' if ok else '🔴'}"
            f" (sep=`{repr(sep)}`, faltan: {faltantes or '–'})\n"
        )
 
    if len(seps) == 2:
        ok = len(set(seps.values())) == 1
        passed = passed and ok
        report_md += (
            f"- Separadores consistentes: {'🟢' if ok else '🔴'}"
            f" ({list(seps.values())})\n"
        )
    ...
    return AssetCheckResult(
        passed=bool(passed),
        severity=AssetCheckSeverity.WARN,
        metadata={"G_COVID2_Cobertura": MetadataValue.md(report_md)},
    )